...
--- @class Augroup
--- @field autocommands Autocommand[] list of autocommands
local Augroup = {
   autocommands = {},
}
...
local function is_valid_target(command)
  return (
          command.targets
      and vim.tbl_islist(command.targets)
...
function scheatkode.augroup(name, commands)
   vim.cmd('augroup ' .. name)
   vim.cmd('autocmd!')

   f.iterate(commands):foreach(function (x)
      if
             x.command
         and x.events
         and is_valid_target(x)
      then
         local command = x.command

         if type(command) == 'function' then
            command = format('lua scheatkode.execute(%d)', scheatkode.register(command))
         end

         vim.cmd(format(
            'autocmd %s %s %s %s',
            table.concat(   x.events,       ','),
            table.concat(  x.targets or {}, ','),
            table.concat(x.modifiers or {}, ' '),
            command
         ))
      ...
      vim.notify(format(
         'An autocommand in %s is specified incorrectly: %s',
         name,
