...
syntax keyword ciscocondition          match eq neq gt lt ge le range
syntax keyword ciscokeyword            speed duplex negotiation delay bandwidth preempt priority timers
syntax keyword ciscokeyword            logging log login password username user license host hostname system
syntax keyword ciscokeyword            address network route neighbor redistribute default-gateway community
syntax keyword ciscokeyword            version class switchport clock name minimum maximum level size
syntax keyword ciscokeyword            established source destination allowed
syntax keyword ciscokeyword            timeout threshold frequency keepalive average weights mtu tunnel
syntax match   ciscokeyword            /timestamps\?/

syntax keyword ciscoprotocol           ipv4 ipv6 tcp udp icmp echo
syntax keyword ciscoprotocol           http https www dhcp domain nameserver ssh telnet ftp ftp-data
syntax keyword ciscoprotocol           ntp snmp snmptrap syslog
syntax keyword ciscoprotocol           smtp pop2 pop3
syntax keyword ciscoprotocol           klogin kshell login rlogin sunrpc
syntax keyword ciscoprotocol           mpls rip isis ospf ospfv3 eigrp bgp hsrp vrrp ipsla
syntax keyword ciscoprotocol           isdn dial hdlc frame-relay atm
syntax keyword ciscoprotocol           igmp multicast broadcast
syntax keyword ciscoprotocol           rsa pki isakmp ipsec ike esp gre vpn mvpn pppoe
syntax keyword ciscoprotocol           qos cef pim ahp tacacs
syntax keyword ciscoprotocol           cdp lldp vtp spanning-tree lacp dot1Q l2tun ethernet
syntax keyword ciscoprotocol           aaa aaa-server
syntax match   ciscoprotocol           /traps\?/

syntax keyword ciscoconfiguration      activate set default redundancy prefe ron tag
syntax keyword ciscoconfiguration      inside outside input output static export import

syntax keyword ciscofunctionality      service crypto encapsulation standby mode in out
syntax keyword ciscofunctionality      storm-control snmp-server group nat banner mask seq metric
syntax keyword ciscofunctionality      add-route shape rd route-target as-path remote-as
syntax keyword ciscofunctionality      access-list access-class access-group prefix-list
syntax keyword ciscofunctionality      passive-interface distribute-list permit subnet-zero
syntax match   ciscofunctionality      /channel\-\(group\|protocol\)/

syntax match   ciscocomment            /!.*$/
syntax match   ciscocomment            /no\s.*$/
syntax match   ciscocomment            /description.*$/
syntax match   ciscocomment            /remark.*$/
syntax match   ciscocomment            /\s*#.*$/

syntax match   ciscostring             /\"[^"]*\"/

syntax match   ciscointerface          /^\(interface\|vlan\|line\|router\|track\)\s.*\d$/
syntax match   ciscointerface          /^ip\s\(sla\|vrf\)\s.*\d$/
syntax match   ciscointerface          /^monitor\ssession\s\d\+$/
syntax match   ciscointerface          /^\(class\|policy\|route\)\-map\s.*$/
syntax match   ciscointerface          /^ip\saccess\-list\s\(standard\|extended\)\s.*$/
syntax match   ciscointerface          /^vrf\s\(definition\|context\)\s.*$/
syntax match   ciscointerface          /^address\-family\sipv.*$/

syntax keyword ciscoaction             disable deny shutdown down none
syntax keyword ciscodanger             password secret privilege

syntax keyword ciscoargument           trunk access full full-duplex auto active monitor
syntax keyword ciscoargument           any enable disable
syntax keyword ciscoargument           pvst mst rapid-pvst \transparent server client
syntax match   ciscoargument           /\d\+[mg]\?/

syntax match   ciscoip                 /\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\(\/[0-9]\{1,2\}\)\?/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{7}\(\:\|\(\x\{1,4}\)\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{6}\(\:\|\(\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\)\|\(\:\x\{1,4}\)\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{5}\(\(\:\(\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\)\?\)\|\(\(\:\x\{1,4}\)\{1,2}\)\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{4}\(\:\x\{1,4}\)\{0,1}\(\:\(\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\)\?\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{4}\(\:\x\{1,4}\)\{0,1}\(\(\:\x\{1,4}\)\{1,2}\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{3}\(\:\x\{1,4}\)\{0,2}\(\:\(\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\)\?\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{3}\(\:\x\{1,4}\)\{0,2}\(\(\:\x\{1,4}\)\{1,2}\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{2}\(\:\x\{1,4}\)\{0,3}\(\:\(\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\)\?\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\{2}\(\:\x\{1,4}\)\{0,3}\(\(\:\x\{1,4}\)\{1,2}\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\(\:\x\{1,4}\)\{0,4}\(\:\(\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\)\?\)/
syntax match   ciscoip6                /\s\(\x\{1,4}\:\)\(\:\x\{1,4}\)\{0,4}\(\(\:\x\{1,4}\)\{1,2}\)/
syntax match   ciscoip6                /\s\:\(\:\x\{1,4}\)\{0,5}\(\(\:\(\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\)\?\)\|\(\(\:\x\{1,4}\)\{1,2}\)\)/

syntax match   ciscorouteip            /\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\(\/[0-9]\{1,2\}\)\?/ contained nextgroup=ciscoroutemask skipwhite
syntax match   ciscoroutemask          /\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\(\.\(25[0-5]\|2[0-4]\d\|[01]\?\d\{1,2}\)\)\{3}\(\/[0-9]\{1,2\}\)\?/ contained nextgroup=ciscointerfacename skipwhite

syntax match   ciscointerfacename      / \d\{1,2}\/\d\{0,2}\/\{0,1}\d\{0,2}/
syntax match   ciscointerfacename      /Loopback\d/
syntax match   ciscointerfacename      /Null0/
syntax match   ciscointerfacename      /\a\{1,15}\d\{1,2}\/\d\{1,2}\/\{0,1}\d\{0,2}:\{0,1}\d\{0,2}/

syntax match   ciscointerface          /interface.*/ contains=ciscointerfacename
syntax match   ciscodescription        /description/ nextgroup=ciscodescriptiontext skipwhite
syntax match   ciscodescriptiontext    /.*/ contained
syntax match   ciscocomment            /^!.*/
