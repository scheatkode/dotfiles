...
describe('sandboxing', function()
	describe('when handling base cases', function()
		...
		it('should have access to safe methods', function()
			...
			assert.same(
				1,
				sandbox.run('local a = {3,2,1}; table.sort(a); return a[1]')
			)
		...
		it('should not allow access to unsafe code', function()
			assert.error(function()
				sandbox.run('return metatable({}, {})')
			end)
			assert.error(function()
				sandbox.run('return string.rep("foobar", 5)')
			end)
		end)
	...
	describe('when handling string.rep', function()
		it('should not allow string:rep', function()
			assert.error(function()
				sandbox.run('return ("hello"):rep(5)')
			end)
		end)
		...
		it('should pass parameters to the sandboxed code', function()
			assert.same(
				3,
				sandbox.run('local a, b = ...; return a + b', {}, 1, 2)
			)
		end)
	end)

	describe(
		'when the sandboxed code attempts to modify the base environment',
		function()
			...
			it('should not persist modifications to base functions', function()
				...
				assert.error(function()
					assert.run('error("this error should be raised")')
				end)
			end)

			it(
				'should not persist modifications to base functions even when provided by the sandboxed environment',
				function()
	...
	describe('when given infinite loops', function()
		...
		it('should accept a quota parameter', function()
			assert.has_no.errors(function()
				sandbox.run('for i = 1, 100 do end')
			end)
			assert.error(function()
				sandbox.run('for i = 1, 100 do end', { quota = 20 })
			end)
		end)
	...
	describe('when given an environment option', function()
		it(
			'should be available to the sandboxed environment as the _G variable',
			function()
				local environment = { foo = 1 }

				assert.same(
					1,
					sandbox.run('return foo', { environment = environment })
				)
				assert.same(
					1,
					sandbox.run('return _G.foo', { environment = environment })
				)
			end
		)
		...
		it('should use the environment metatable, if it exists', function()
			...
			assert.same(
				3,
				sandbox.run('return foo + bar', { environment = env2 })
			)
		end)
		...
		it('should override the base environment with false', function()
			...
			assert.same(
				false,
				sandbox.run('return tostring', { environment = environment })
			)
		end)
	end)
end)
